...
class SegData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dataname=="cov19": #QaTa-COV19
            ...
            imageRaw = os.path.join(self.root_path,'Images',self.image_list[idx].replace('mask_',''))#Raw
        ...
        data = {'image':image,'image2':image2, 'imageRaw':imageRaw, 'gt':gt, 'token':token, 'mask':mask}
        data = trans(data)

        image,image2,imageRaw,gt,token,mask = data['image'],data['image2'],data['imageRaw'],data['gt'],data['token'],data['mask']
        ...
        return ([image,image2,imageRaw, text], gt)

    def transform(self,image_size=[224,224]):

        keys = ["image","image2", "imageRaw", "gt"]
        if self.mode == 'train':  
            trans = Compose([
                LoadImaged(keys=keys, reader='PILReader'),  
                EnsureChannelFirstd(keys=keys),  
                RandZoomd(keys=keys, min_zoom=0.95, max_zoom=1.15, mode=["bicubic","bicubic","bicubic","nearest"], prob=0.3),  
                RandRotated(keys=keys, range_x=[-0.3, 0.3], keep_size=True, mode=['bicubic','bicubic','bicubic','nearest'],  prob=0.3),  
                RandGaussianNoised(keys=["image2"], prob=0.3, mean=0.0, std=0.1),
                RandGaussianNoised(keys=["imageRaw"], prob=0.3, mean=0.0, std=0.1),
                Resized(["image"],spatial_size=image_size,mode='bicubic'),
                Resized(["image2"],spatial_size=image_size,mode='bicubic'),
                Resized(["imageRaw"], spatial_size=image_size, mode='bicubic'), 
                Resized(keys=["gt"], spatial_size=image_size, mode='nearest'), 
                NormalizeIntensityd(keys=["image"], channel_wise=True),
                NormalizeIntensityd(['image2'], channel_wise=True),
                NormalizeIntensityd(['imageRaw'], channel_wise=True),
                ToTensord(keys=["image","image2", "imageRaw", "gt", "token", "mask"])
            ])
        else:
            trans = Compose([
                LoadImaged(["image","image2","imageRaw","gt"], reader='PILReader'),
                EnsureChannelFirstd(["image","image2","imageRaw","gt"]),
                Resized(["image"],spatial_size=image_size,mode='bicubic'),
                Resized(["image2"],spatial_size=image_size,mode='bicubic'),
                Resized(["imageRaw"], spatial_size=image_size, mode='bicubic'),
                ...
                NormalizeIntensityd(['image2'], channel_wise=True),
                NormalizeIntensityd(['imageRaw'], channel_wise=True),
                ToTensord(["image","image2","imageRaw","gt","token","mask"]),
            ])
